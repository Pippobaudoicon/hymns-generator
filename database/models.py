...
class Ward(Base):
    ...
    hymn_selections = relationship("HymnSelection", back_populates="ward", cascade="all, delete-orphan")
    
    # Many-to-many relationship with users
    assigned_users = relationship(
        "User",
        secondary="user_ward_assignments",
...
class HymnSelection(Base):
    ...
    hymns = relationship("SelectedHymn", back_populates="selection", cascade="all, delete-orphan")
...
class SelectedHymn(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    selection_id = Column(Integer, ForeignKey("hymn_selections.id", ondelete="CASCADE"), nullable=False)
