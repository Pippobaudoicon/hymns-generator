...
from .models import Hymn, FestivityType, HymnFilter
...
class HymnService:
    ...
    def _exclude_festive_hymns(self, hymns: List[Hymn], allowed_festivity: Optional[str] = None) -> List[Hymn]:
        """
    ...
    def _get_sacramento_hymns(self, domenica_festiva: bool = False, tipo_festivita: Optional[FestivityType] = None) -> List[Hymn]:
        ...
        if domenica_festiva and tipo_festivita:
            ...
            sacramento_hymns = self._exclude_festive_hymns(sacramento_hymns, tipo_festivita.value)
        else:
    ...
    def _get_other_hymns(self, domenica_festiva: bool, tipo_festivita: Optional[FestivityType]) -> List[Hymn]:
        ...
        if domenica_festiva:
            if not tipo_festivita:
                raise InvalidFilterError("tipo_festivita is required when domenica_festiva is true")
            
            # Start with festive hymns by category
            festive_hymns_by_category = self._filter_by_category(tipo_festivita.value)
            festive_hymns_by_category = [h for h in festive_hymns_by_category if h.category.lower() != "sacramento"]
            
            # Add festive hymns by tags (from other categories)
            festive_hymns_by_tags = self._filter_by_tags(tipo_festivita.value)
            festive_hymns_by_tags = [h for h in festive_hymns_by_tags if h.category.lower() != "sacramento" and h.category.lower() != tipo_festivita.value]
            ...
            for hymn in festive_hymns:
                if hymn.number not in seen_numbers and hymn.category.lower() != "sacramento":
            ...
            other_hymns = self._exclude_festive_hymns(unique_festive_hymns, tipo_festivita.value)
            ...
            if len(other_hymns) < required_hymns:
                ...
                logger.warning(f"Only {len(other_hymns)} festive hymns available, adding {hymns_needed} from occasioni speciali")
                ...
                for hymn in special_occasion_hymns:
                    if hymn.number not in seen_numbers and added_count < hymns_needed:
                        ...
                        elif tipo_festivita.value == "pasqua" and "natale" in tags_lower:
                            has_conflicting_tags = True
    ...
    def get_hymns(
        ...
        try:
            ...
            sacramento_hymns = self._get_sacramento_hymns(domenica_festiva, tipo_festivita)
    ...
    def get_hymn(self, hymn_filter: HymnFilter) -> Optional[Hymn]:
        ...
        try:
            ...
            if hymn_filter.number is not None:
                filtered_hymns = [h for h in filtered_hymns if h.number == hymn_filter.number]
            
            if hymn_filter.category is not None:
                filtered_hymns = [h for h in filtered_hymns 
            ...
            logger.info(f"Selected hymn: {selected_hymn.title} (#{selected_hymn.number})")
