...
user_ward_association = Table(
    ...
    Column(
        "ward_id", Integer, ForeignKey("wards.id", ondelete="CASCADE"), nullable=False
    ),
...
class Area(Base):
    ...
    managers = relationship(
        "User", back_populates="managed_area", foreign_keys="User.area_id"
    )
...
class Stake(Base):
    ...
    managers = relationship(
        "User", back_populates="managed_stake", foreign_keys="User.stake_id"
    )
...
class User(Base):
    ...
    # Relationships
    managed_area = relationship(
        "Area", back_populates="managers", foreign_keys=[area_id]
    )
    managed_stake = relationship(
        "Stake", back_populates="managers", foreign_keys=[stake_id]
    )

    # Many-to-many relationship with wards (for ward users)
    assigned_wards = relationship(
        "Ward", secondary=user_ward_association, back_populates="assigned_users"
    )
