...
def create_stake(
    ...
    current_user: User = Depends(require_role([UserRole.SUPERADMIN, UserRole.AREA_MANAGER])),
    db: Session = Depends(get_database_session),
) -> StakeResponse:
    """
    Create a new stake.
    ...
    try:
        ...
        if stake_data.area_id:
            ...
            if user_role == UserRole.AREA_MANAGER and current_user.area_id != stake_data.area_id:
                raise HTTPException(
                    status_code=403,
...
@router.get("/stakes/{stake_id}", response_model=StakeResponse, summary="Get stake by ID")
def get_stake(
    stake_id: int,
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_database_session),
...
@router.put("/stakes/{stake_id}", response_model=StakeResponse, summary="Update a stake")
def update_stake(
    stake_id: int,
    ...
    try:
        ...
        if stake_data.name:
            existing = db.query(Stake).filter(Stake.name == stake_data.name, Stake.id != stake_id).first()
            if existing:
    ...
    try:
        ...
        for ward in stake.wards:
            ...
            for selection in ward.hymn_selections:
                db.query(SelectedHymn).filter(SelectedHymn.selection_id == selection.id).delete()
...
@router.get("/areas/{area_id}/stakes", response_model=List[StakeResponse], summary="Get stakes in an area")
def get_area_stakes(
    area_id: int,
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_database_session),
...
def update_ward(
    ...
    if "name" in ward_data and ward_data["name"]:
        ...
        existing = db.query(Ward).filter(Ward.name == ward_data["name"], Ward.id != ward_id).first()
        if existing:
            raise HTTPException(status_code=400, detail="Ward with this name already exists")
