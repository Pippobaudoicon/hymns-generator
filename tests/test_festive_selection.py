...
def test_festive_selection_multiple_times():
    ...
    for i in range(10):
        ...
        for hymn in hymns:
            if hymn.category.lower() == 'natale':
                print(f"  Attempt {i+1}: Found natale hymn #{hymn.number}: {hymn.title}")
    ...
    for i in range(10):
        ...
        for hymn in hymns:
            if hymn.category.lower() == 'pasqua':
                print(f"  Attempt {i+1}: Found pasqua hymn #{hymn.number}: {hymn.title}")
    ...
    # Christmas
    other_hymns = service._get_other_hymns(domenica_festiva=True, tipo_festivita=FestivityType.NATALE)
    natale_in_pool = []
    for hymn in other_hymns:
        if hymn.category.lower() == 'natale' or 'natale' in [tag.lower() for tag in hymn.tags]:
    ...
    for hymn in natale_in_pool:
        ...
        print(f"  #{hymn.number}: {hymn.title} (Category: {hymn.category}) [from {source}]")
    
    # Easter
    other_hymns = service._get_other_hymns(domenica_festiva=True, tipo_festivita=FestivityType.PASQUA)
    pasqua_in_pool = []
    for hymn in other_hymns:
        if hymn.category.lower() == 'pasqua' or 'pasqua' in [tag.lower() for tag in hymn.tags]:
    ...
    sacramento_natale = service._get_sacramento_hymns(domenica_festiva=True, tipo_festivita=FestivityType.NATALE)
    natale_sacramento = []
    for hymn in sacramento_natale:
        if hymn.category.lower() == 'natale' or 'natale' in [tag.lower() for tag in hymn.tags]:
            natale_sacramento.append(hymn)
    
    print(f"Sacramento hymns with natale category/tag available for Christmas: {len(natale_sacramento)}")
    for hymn in natale_sacramento:
        source = "category" if hymn.category.lower() == 'natale' else "tag"
        print(f"  #{hymn.number}: {hymn.title} [from {source}]")
    
    # Easter Sacramento pool
    sacramento_pasqua = service._get_sacramento_hymns(domenica_festiva=True, tipo_festivita=FestivityType.PASQUA)
    pasqua_sacramento = []
    for hymn in sacramento_pasqua:
        if hymn.category.lower() == 'pasqua' or 'pasqua' in [tag.lower() for tag in hymn.tags]:
            pasqua_sacramento.append(hymn)
    
    print(f"Sacramento hymns with pasqua category/tag available for Easter: {len(pasqua_sacramento)}")
